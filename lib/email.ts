...
export async function sendEmail(
  ...
  try {
    ...
    if (!process.env.SMTP_USERNAME || !process.env.SMTP_PASSWORD) {
      console.log(
        "[EMAIL] SMTP not configured in environment variables, skipping email"
      );
      return false;
    }
    ...
    try {
      ...
      console.log(
        `[EMAIL] Email sent successfully to ${options.to} - Subject: ${options.subject}`
      );
      return true;
    } catch (error) {
...
export async function sendBackupNotification(
  ...
  try {
    ...
    if (recipients.length === 0) {
      console.log(
        `[EMAIL] No email recipients configured in environment variables`
      );
      return;
    }

    console.log(
      `[EMAIL] Sending ${notificationType} notification to ${recipients.length} recipients`
    );
    ...
    console.log(
      `[EMAIL] Backup notification sent successfully to ${successCount}/${recipients.length} recipients`
    );
  } catch (error) {
            ...
            <div style="padding: 20px;">
              <div style="background: #f0fdf4; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #22c55e;">
                ...
                <table style="width: 100%; border-collapse: collapse;">
                  ...
                  <tr><td style="padding: 5px 0; font-weight: bold;">Dosya Adı:</td><td style="padding: 5px 0; font-family: monospace; background: #f3f4f6; padding: 2px 6px; border-radius: 4px;">${
                    data.fileName
                  }</td></tr>
                  <tr><td style="padding: 5px 0; font-weight: bold;">Dosya Boyutu:</td><td style="padding: 5px 0;">${formatFileSize(
                    data.fileSize
                  )}</td></tr>
                  <tr><td style="padding: 5px 0; font-weight: bold;">Kaynak Klasör:</td><td style="padding: 5px 0;">${
                    data.sourceFolders
                  } adet</td></tr>
                  <tr><td style="padding: 5px 0; font-weight: bold;">İşlem Süresi:</td><td style="padding: 5px 0;">${Math.round(
                    data.processingTime / 1000
                  )} saniye</td></tr>
                  ${
                    data.deletedOldBackups > 0
                      ? `<tr><td style="padding: 5px 0; font-weight: bold;">Silinen Eski Yedek:</td><td style="padding: 5px 0;">${data.deletedOldBackups} adet</td></tr>`
                      : ""
                  }
                </table>
              </div>
...
${
  data.deletedOldBackups > 0
    ? `Silinen Eski Yedek: ${data.deletedOldBackups} adet`
    : ""
}
            ...
            <div style="padding: 20px;">
              <div style="background: #fef2f2; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #ef4444;">
                ...
                <table style="width: 100%; border-collapse: collapse;">
                  ...
                  <tr><td style="padding: 5px 0; font-weight: bold; vertical-align: top;">Hata:</td><td style="padding: 5px 0; font-family: monospace; background: #fee2e2; padding: 8px; border-radius: 4px; color: #991b1b;">${
                    data.error
                  }</td></tr>
                </table>
              </div>
            ...
            <div style="padding: 20px;">
              <div style="background: #f8fafc; padding: 20px; border-radius: 8px; margin: 20px 0;">
                <table style="width: 100%; border-collapse: collapse;">
                  ...
                  <tr><td style="padding: 8px 0; font-weight: bold;">Toplam Konfigürasyon:</td><td style="padding: 8px 0;">${
                    data.totalConfigs
                  }</td></tr>
                  <tr><td style="padding: 8px 0; font-weight: bold;">Başarılı:</td><td style="padding: 8px 0; color: #22c55e; font-weight: bold;">${
                    data.successCount
                  }</td></tr>
                  <tr><td style="padding: 8px 0; font-weight: bold;">Başarısız:</td><td style="padding: 8px 0; color: #ef4444; font-weight: bold;">${
                    data.errorCount
                  }</td></tr>
                  <tr><td style="padding: 8px 0; font-weight: bold;">Toplam İşlem Süresi:</td><td style="padding: 8px 0;">${Math.round(
                    data.totalProcessingTime / 1000
                  )} saniye</td></tr>
                </table>
              </div>
              ...
              ${data.results
                ...
                <div style="background: ${
                  result.success ? "#f0fdf4" : "#fef2f2"
                }; padding: 15px; border-radius: 6px; margin: 10px 0; border-left: 4px solid ${
                    result.success ? "#22c55e" : "#ef4444"
                  };">
                  <h4 style="margin: 0 0 10px 0; color: ${
                    result.success ? "#166534" : "#991b1b"
                  };">${result.success ? "✅" : "❌"} ${result.configName}</h4>
                  ${
                    result.success
                      ? `
                    <p style="margin: 5px 0; font-size: 14px;"><strong>Dosya:</strong> <code style="background: #f3f4f6; padding: 2px 4px; border-radius: 3px;">${
                      result.fileName
                    }</code></p>
                    <p style="margin: 5px 0; font-size: 14px;"><strong>Boyut:</strong> ${formatFileSize(
                      result.fileSize
                    )}</p>
                    <p style="margin: 5px 0; font-size: 14px;"><strong>Kaynak Klasör:</strong> ${
                      result.sourceFolders
                    } adet</p>
                    ${
                      result.deletedOldBackups > 0
                        ? `<p style="margin: 5px 0; font-size: 14px;"><strong>Silinen Eski Yedek:</strong> ${result.deletedOldBackups} adet</p>`
                        : ""
                    }
                  `
                      : `
                  ...
                  <p style="margin: 5px 0; font-size: 14px;"><strong>Süre:</strong> ${Math.round(
...
${
  result.success
    ? `  Dosya: ${result.fileName}\n  Boyut: ${formatFileSize(
        result.fileSize
      )}\n  Kaynak Klasör: ${result.sourceFolders} adet${
        result.deletedOldBackups > 0
          ? `\n  Silinen Eski Yedek: ${result.deletedOldBackups} adet`
          : ""
      }`
    : `  Hata: ${result.error}`
}
