...
// Fixed: Use user-specific endpoint for backup operations
async function compressMultipleFolders(accessToken: string, sourcePaths: string[]): Promise<Buffer> {
  ...
  for (const folderPath of sourcePaths) {
    try {
      ...
      if (!response.ok) {
        console.warn(`Klasör erişim hatası: ${folderPath}`)
        continue
      }
      ...
      for (const item of folderData.value) {
        if (item.file) {
          try {
            ...
            console.warn(`Dosya indirme hatası: ${item.name}`)
          }
        }
      }
    } catch (folderError) {
      console.warn(`Klasör işleme hatası: ${folderPath}`, folderError)
    }
  }
