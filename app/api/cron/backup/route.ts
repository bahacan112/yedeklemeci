...
import { verifyUser, getBackupConfigsByUser, createBackupHistory, cleanupOldBackups, updateBackupHistory } from '@/lib/database'
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const config of configs) {
      ...
      try {
        console.log(`[CRON] Processing config: ${config.name} (ID: ${config.id})`)
        ...
        try {
          ...
          for (const oldBackup of oldBackups) {
            try {
              ...
              console.error(`[CRON] Error deleting old backup: ${oldBackup.file_name}`, deleteError)
            }
          }
          ...
          console.log(`[CRON] Successfully completed backup for config: ${config.name}`)
    ...
    console.log(`[CRON] Backup process completed for user: ${username}. Success: ${successCount}, Errors: ${errorCount}`)
      ...
      for (const item of folderData.value) {
        if (item.file) {
          try {
            ...
            console.warn(`[CRON] Dosya indirme hatası: ${item.name}`)
          }
        }
      }
    } catch (folderError) {
